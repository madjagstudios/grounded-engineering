...
reviewable write surface. The pinned source is the official Claude Code
...
`CLAUDE.local.md` files as context, but v0.4.0 does not modify them. A detected
`AGENTS.md` is reported as an existing provider surface; the Claude adapter
does not automatically import or rewrite it. This avoids silently changing
provider precedence and keeps shared guidance a local review decision.
...
The baseline pack's release metadata and the new AI-assisted pack's release
metadata must identify v0.4.0. Pack and manifest versions remain explicit and
schema-validated. Public card dispositions remain separate from consuming
...
Implement the existing `grounded-engineering check` command. It reads the
repository's `.grounded-engineering/manifest.yaml`, the selected local pack,
the selected cards, and the target files. It never fetches sources, writes
files, creates proposals, or changes the manifest.
...
managed-block mismatch returns a nonzero status and names the failed lane. The
human-readable output reports a concise status and actionable diagnostics; it
does not claim that a clean managed block proves the repository's broader
policy is correct.
...
- `src/lib/preflight.mjs` chooses root `CLAUDE.md` and reports nearby Claude
  instruction surfaces without writing them.
...
- `src/cli.mjs` routes `check` to the checker and preserves usage handling for
  unsupported options.
...
- The package version, README, changelog, pack metadata, release notes, and
  command help consistently describe v0.4.0.
